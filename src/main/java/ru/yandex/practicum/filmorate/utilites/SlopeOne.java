...
/**
 * Класс SlopeOne реализует одноимённый алгоритм коллаборативной фильтрации класса "item-based filtration" -
 * фильтрация по схожести предметов (Item), сравнивающая оценки, данные различными пользователями (User).
 *
 * @param <U> User (Пользователь, или сущность, дающая оценку Предмету).
 * @param <I> Item (Предмет, или сущность, которая может иметь оценку Пользователя).
 * @param <T> Number type (число, наследующее Number, являющееся оценкой Пользователя Предмету).
 */
@Slf4j
public class SlopeOne<U, I, T extends Number> {
    ...
    /**
     * Конструктор класса SlopeOne, в который следует передать два параметра/набора данных:
     *
     * @param targetUserRates     - оценки некоторого целевого пользователя, для которого подбираются рекомендации.
     * @param similarUserRatesMap - карта оценок прочих пользователей, выставивших свои оценки предметам.
     */
    public SlopeOne(Map<I, T> targetUserRates, Map<U, Map<I, T>> similarUserRatesMap) {
    ...
    /**
     * Метод, возвращающий результат работы алгоритма:
     *
     * @return список рекомендуемых Предметов для целевого пользователя.
     */
